...
2. **获取 RSS**: 按优先级抓取 AI/GovTech/BigTech/互联网资讯/奇思妙想/技术团队与安全/产品与架构动态（仅过去 24 小时）
3. **扫描状态**: 检查任务、项目、战略文件
...
问题 2 选择后必须进行一轮追问，采集每个已选项的具体内容（不能只保留标签）。
...
问题 3 选择后必须进行追问，采集可落盘的具体信息（不能只保留标签）。
