...
- **禁止修改状态**
- 机会点建议先进入 Inbox
- 必须写入缓存文件 `60_Resources/Newsletters/YYYY-MM/YYYY-MM-DD-digest.md`
...
执行规则：
- 先确保目录存在：`60_Resources/Newsletters/YYYY-MM/`
- 若当天缓存存在且非空：优先复用缓存，再生成/更新 brief
- 若当天缓存不存在：完成抓取后必须落盘缓存文件，再生成 brief
- `/morning` 执行结束时，`brief` 与 `digest` 两个文件必须同时存在
